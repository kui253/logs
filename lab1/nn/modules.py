...
class Linear(Module):
    ...
    def forward(self, x):
        ...
        # of linear module.
        self.x0 = x
        sig = np.dot(x,self.w[1:])+self.w[0]
        self.sigma = 1/(1+np.exp(-sig))
        return self.sigma
        # End of todo
    ...
    def backward(self, dy):
        ...
        # of linear module.
        dx = np.zeros((dy.shape[0],2))
        self.w.grad = np.zeros((3,1))
        dx += self.w[1:].T
     
        temp2 = np.sum(self.x0*dy,axis=0)/100
        self.w.grad[0] = np.sum(dy)/100
        self.w.grad[1:3] = np.reshape(temp2,(2,1))
        return dx
        # End of todo
